...
describe('Pool', () => {
  ...
  test('get', () => {
    ...
    console.log(pool.top(7));
    expect(pool.get(0).weight).toEqual(99);
    expect(pool.get(101)).toBeUndefined();
  });
  //test('find', () => {
    //const pool = createPool();
    //expect(pool.find('foo')).toEqual(-1);
    //expect(pool.find('k99')).toEqual(0);
  ...
  //test('val', () => {
    //const pool = createPool();
    //expect(pool.val(0)).toEqual('k99');
    //expect(pool.val(100)).toBeUndefined();
  //});
  //test('peek', () => {
    //const pool = createPool();
  ...
  //test('top', () => {
    ...
    //expect(pool.top(1).map(n => n.weight)).toEqual([99]);
    //expect(pool.top(5).map(n => n.weight)).toEqual([99, 98, 97, 96, 95]);
  //});
});
